...
Future<bool> checkInternetAccess() async {
  ...
  try {
    Response response = await dio.get(url).timeout(Duration(seconds: 100)).catchError((e) {
      throw e;
    });
...
Future<bool> getCategories(AppBloc appBloc) async {
  ...
  try {
    Response response = await dio.get(
      "$urlApi/Pedido/obter-categorias"
    ).timeout(Duration(seconds: 200)).catchError((e) {
      throw e;
    });
    if (response.statusCode == 200) {
...
Future<bool> getUser(AppBloc appBloc, {String credential}) async {
  ...
  try {
    Response response = await dio.get(
        "$urlApi/Cliente/obter-cliente?Credencial=$credential"
    ).timeout(Duration(seconds: 200)).catchError((e) {
      throw e;
    });
...
Future<bool> getEnderecosService(AppBloc appBloc) async {
  ...
  try {
    Response response = await dio.get(
      ...
      print("$urlApi/Endereco/obter-endereco?clienteId=${appBloc.userModel.clienteId}");
      throw e;
    });
    if (response.statusCode == 200) {
...
Future<bool> getCompaniesService(AppBloc appBloc, {String lat, String lng}) async {
  Dio dio = Dio();

  try {
    Response response = await dio.post(
        "$urlApi/Pedido/obter-lojas",
      data: {
        "Latitude": lat,
        "Longitude": lng
      }
...
Future<bool> setPedidoService(AppBloc appBloc, {
  ...
  try {
    Response response = await dio.post(
        "$urlApi/Pedido/finalizar-pedido",
        data: 
          {
            ...
            "Endereco": "${enderecoModel.logradouro}, ${enderecoModel.numero}",
